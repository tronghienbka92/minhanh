...
namespace Nop.Services.NhaXes
{
    public class LimousineBanVeService : ILimousineBanVeService
    {
        ...
        public virtual ListDoanhThuTongHop GetDoanhThuTongHopTheoNgay(DateTime NgayDi, int NhaXeId)
        {
            ...
            var TNhanhtrinhids = _hanhtrinhkhuvucRepository.Table.Where(c => c.KhuVucId == (int)ENKhuVuc.THAI_NGUYEN).Select(c => c.HanhTrinhId);
            var chuyendiHN = query.Where(c => HNhanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();
            var chuyendiTN = query.Where(c => TNhanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();
            //doanh thu ha noi
           item.DT_HA_NOI = new DoanhThuTongHop();
           var datvesnb = _datveRepository.Table.Where(c => chuyendiHN.Contains(c.ChuyenDiId.Value)
                && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
           item.DT_HA_NOI.TongLuot = datvesnb.Select(c => c.ChuyenDiId).Distinct().Count();
           item.DT_HA_NOI.SoKhach = datvesnb.Count();
           item.DT_HA_NOI.ThanhTien = 0;
           if (datvesnb.Count() > 0)
           {
               item.DT_HA_NOI.ThanhTien = datvesnb.Sum(c => c.GiaTien);
           }
           //doanh thu thai nguyen
           item.DT_THAI_NGUYEN = new DoanhThuTongHop();
           var datvetkdns = _datveRepository.Table.Where(c => chuyendiTN.Contains(c.ChuyenDiId.Value)
                && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
           item.DT_THAI_NGUYEN.TongLuot = datvetkdns.Select(c => c.ChuyenDiId).Distinct().Count();
           item.DT_THAI_NGUYEN.SoKhach = datvetkdns.Count();
           item.DT_THAI_NGUYEN.ThanhTien = 0;
           if (datvetkdns.Count() > 0)
           {
               item.DT_THAI_NGUYEN.ThanhTien = datvetkdns.Sum(c => c.GiaTien);
           }
